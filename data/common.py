...
def get_patch(*args, patch_size=96, scale=[2], multi_scale=False):
    ...
    tp = math.floor(math.floor(patch_size // scale[0]) * scale[0])  # patch size of target hr image
    ip = [math.floor(patch_size // s) for s in scale] # patch size of lr images

    # tx and ty are the top  and left coordinate of the patch 
    tx = random.randrange(0, tw - tp + 1)
    ty = random.randrange(0, th - tp + 1)
    tx, ty = math.floor(tx- tx % scale[0]), math.floor(ty - ty % scale[0])
    ix, iy = [ math.floor(tx // s) for s in scale], [math.floor(ty // s) for s in scale]
